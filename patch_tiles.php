...
/**
 * Tile row: closer, narrower, whole tiles per view + the custom scroll
 * navigation. Plus the favicon link.
 *
 * Files: public/index.php (markup), public/assets/css/app.css (geometry),
 * public/assets/js/app.js (the navigation behaviour), src/helpers/translations.php
 * (the three accessible labels). Nothing else.
 */
...
patch($root . '/public/index.php', [
    /* The favicon: the browser icon file, never a category icon. */
    [
        <<<'HTML'
...
HTML,
    ...
    /* The scroller and its navigation. */
    [
        <<<'HTML'
...
HTML,
        <<<'HTML'
                <!--
                    The tile row and its navigation. The row is a horizontal
                    scroller: it shows whole tiles only (the count per view lives
                    in the stylesheet) and snaps to a tile, so a tile is never
                    left half cut off. app.js fills the counter and the thumb and
                    keeps the whole navigation hidden while every tile fits.
                -->
                <div class="tiles" id="tiles">
                    <div class="area-grid" id="area-grid"></div>

                    <div class="tiles-nav" id="tiles-nav" hidden>
                        <p class="tiles-nav__counter" id="tiles-nav-counter"
                           data-i18n-label="scroller.position"></p>
...
CSS,
    ...
    /* The tile geometry the request asked for. */
    [
        <<<'CSS'
    gap: 10px;
    padding: 28px;
...
CSS,
    ...
    /* The navigation, in the design tokens of the sheet - no new colour. */
    [
        <<<'CSS'
...
CSS,
    ...
    /* Whole tiles per view: 4 desktop, 3 laptop, 2 tablet, 1 phone. */
    [
        <<<'CSS'
/*
...
CSS,
    ...
    /* The old two-column rule is gone: there are no rows to stack any more. */
    [
        <<<'CSS'
...
CSS,
    ...
    /* Reduced motion: no smooth travelling of the row. */
    [
        <<<'CSS'
...
patch($root . '/public/assets/js/app.js', [
    /* Element handles for the navigation. */
    [
        <<<'JS'
...
JS,
    ...
    /* The row no longer switches between a grid and a strip: it is always the
       scroller, and it simply does not scroll while everything fits. */
    [
        <<<'JS'
            /*
...
JS,
    ...
    /* The module itself, right before the linked-hover wiring. */
    [
        <<<'JS'
...
JS,
    ...
    /* Language switches move the labels and the counter. */
    [
        <<<'JS'
...
/* ==========================================================================
   4. translations.php - the accessible labels of the navigation
   ========================================================================== */
